...
df$V6 <- NULL
#create a clone data frame of the raw data, to get a better aligned raw data and not change the original raw data.
...
sensorydata_tidy_br <- data.frame(Item=rep(new_sensorydata_raw$Item, 5), as.character(Operator[,2]), as.numeric(Operator[,1]))
...
boxplot(sensorydata_tidy_br$sensorydata~sensorydata_tidy_br$Item, xlab = "Item", ylab = "Sensory Data",main="Sensory data from five operators")
```
...
as.data.frame(head(new_sensorydata_raw))
 ...
 sensorydata_tidy_tv <- 
  new_sensorydata_raw %>%
   gather(key = "Operators", value = "sensorydata", Operator1, Operator2, Operator3, Operator4,       Operator5)
...
boxplot(sensorydata_tidy_tv$sensorydata~sensorydata_tidy_tv$Item, xlab = "Item", ylab = "sensorydata", main="Sensory data from five operators")
```
...
```{r plot_longjump_baseR, include=TRUE,echo=FALSE}
plot(longjump_tidy_br$Year, longjump_tidy_br$LongJumpData,cex= 0.4, xlab = "Year", ylab = "LongJump Performance", main = "Gold Medal performance for Olympic Men’s Long Jump")
```
...
Then our plot of year to Gold Medal performance for Olympic Men’s Long Jump is:
```{r plot_longjump_tidyverse, include=TRUE,echo=FALSE}
plot(longjump_tidy_tv$Year, longjump_tidy_tv$LongJumpData,cex= 0.4, xlab = "Year", ylab = "LongJump Performance", main = "Gold Medal performance for Olympic Men’s Long Jump")
```
...
#create a dataframe called tomato_tidy where there is 3 of the tomatovariety, the string of their measurements, and densities.
...
# bind the density, m number, and measurements of the two types of tomatoes and reoder the rows of the dataframe to look more like the tidyverse dataframe(since I did that first) for easy comparison reason.
tomato_tidy_br <- as.data.frame(cbind(subset.df, new.df))
...
boxplot(tomato_tidy_br$Ife1~tomato_tidy_br$Density, xlab = "Density", ylab = "Tomato yield for Type I of tomato(Ife1)", main = "Measurements of tomato yield for tomato(Ife1)")
```
